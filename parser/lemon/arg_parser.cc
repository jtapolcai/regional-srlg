...
namespace lemon {
  ...
  void ArgParser::shortHelpPhp() const
  {
   std::cout<<"<span class=\"header\">Parameters:</span>\n<table>\n <tr> <td>The name of simulation:</td>\n";
   std::cout<<"<td><input type=\"text\" name=\"sim_name\" value=\"<?php echo gettimestamp(); ?>\"/> </td>\n</tr>\n";
//<?php param_array_to_form_table($lgf_params); ?>
//<?php param_array_to_form_table($trf_params); ?>
    int pos=_command_name.size()+2;
    for(Groups::const_iterator g=_groups.begin();g!=_groups.end();++g) {
      std::string name=g->first;
      std::cout<<"<tr><td>"<<name<<"</td><td><select name=\""<<name<<"\">\n";
      for(GroupData::Opts::const_iterator o=g->second.opts.begin();
          o!=g->second.opts.end();++o){
        Opts::const_iterator i=_opts.find(*o);
        std::cout<<"<option value=\""<<i->first<<"\">"<<i->second.help<<"</option>\n";
/*    switch(i->second.type) {
    case STRING:
      os << " str";
      break;
    ...
    default:
      ...
      std::cout<<"</select></td></tr>\n";
    }
    for(Opts::const_iterator i=_opts.begin();i!=_opts.end();++i)
      if(!i->second.ingroup&&!i->second.syn) {
        std::cout<<"</td><td><input type=\""<<i->second.help<<"\" name=\""<<i->first<<"\" ";
//        std::cout<<" value=\""<<operator[](i->first);
        std::cout<<"\"/></td></tr>\n";
//        if(!i->second.mandatory) cstr << '[';
      }
    for(std::vector<OtherArg>::const_iterator i=_others_help.begin();
