...
CPXwriteprob (cplexEnv(), _prob, "cplex_problem.lp", "LP"); 
    _clear_temporals();
    _applyMessageLevel();
  ...
  double gap_limit = 0.02; 
  if (gap_limit > 0)
      CPXsetdblparam(cplexEnv(), CPX_PARAM_EPGAP, gap_limit);
/*	
 double time_limit = 240.0;

 if (time_limit>0)
      CPXsetdblparam(cplexEnv(), CPX_PARAM_TILIM,time_limit*60.0);
*/
